...
Os três primeiros desafios propostos neste README foram resolvidos nos seguintes arquivos:

## 1 - Concatenando Dados

Arquivo: [`concat_dados.py`](concat_dados.py)

O programa recebe dois dados digitados pelo usuário, concatena os valores em uma única string e exibe o resultado.
...
Arquivo: [`repet_txt.py`](repet_txt.py)

O programa recebe um texto e um número inteiro e, em seguida, exibe o texto repetido pela quantidade informada. O programa também verifica se a quantidade digitada é válida.
...
Arquivo: [`ope_mat.py`](ope_mat.py)

O programa recebe dois números e permite escolher entre as operações de soma (`+`), subtração (`-`), multiplicação (`*`) e divisão (`/`). Entradas inválidas e divisão por zero são tratadas pelo programa.
